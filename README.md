...
| 指标 | logx | 说明 |
|------|:--:|------|
| TextEncoder 简单日志（编码路径） | **22.1 ns/op, 0 B, 0 allocs** | 零分配纯字节拼接 |
| TextEncoder 含字段（编码路径） | **54.2 ns/op, 0 B, 0 allocs** | 强类型字段分发 |
| 端到端（io.Discard，3 字段） | **122.8 ns/op, 0 B, 0 allocs** | Entry 池化 + 内联字段容器 |
| 文件异步写入（3 字段） | **230.6 ns/op, 0 allocs** | 槽位复用 + 有界背压 |
| 文件同步写入（3 字段） | **2927 ns/op, 0 allocs** | 直接落盘 |
| 未启用级别过滤 | **55.9 ns/op, 0 allocs** | 仅级别判断开销 |

### 吞吐量（`examples/bench`）

| 场景 | 吞吐量 |
|------|--:|
| 文件异步写入 | **4,952,640 条/s** |
| 文件同步写入 | 370,622 条/s |
| 文件异步 + 3 字段 | 3,787,414 条/s |
| 未启用级别过滤 | **20,118,049 条/s** |
